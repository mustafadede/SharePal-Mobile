...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    getSelectedUserWatched(profile.userId).then((watched) => {
      ...
      const filteredMovieData = watched?.filter((item) => item.mediaType === "movie");
  ...
  const handleCurrentlyWatching = () => {
    useSearchWithYear(profile.currentlyWatching.title, profile.currentlyWatching.releaseDate?.slice(0, 4)).then((item) => {
      router.push({
